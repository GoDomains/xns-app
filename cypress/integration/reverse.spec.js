...
describe(
  ...
  () => {
    it('is set', () => {
      ...
      cy.queryByText(`Primary XNS Name (reverse record)`, {
        exact: false,
        timeout: 10000
      })
      ...
      cy.getByText('Select one of your XNS names', { exact: false }).click({
        force: true
      })

      cy.getByText('sub1.otherowner.xdc', { exact: true }).click({
        force: true
      })

      cy.getByText('Save', { timeout: 5000 }).click({ force: true })

      cy.queryByText(`Primary XNS Name (reverse record): sub1.otherowner.xdc`, {
        exact: false,
        timeout: 10000
      }).should('exist')
