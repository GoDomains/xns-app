...
describe('/address', () => {
  it('does not contain unnormalised names', () => {
    ...
    cy.queryByText('InvalidName.go', { exact: false, timeout: 5000 }).should(
      'not.exist'
    )
  })
  ...
  it('contains the list of names owened by the user', () => {
    ...
    cy.queryByText('newname.go', { exact: false }).should('exist')
  ...
  it('can select a name', () => {
    ...
    cy.getByTestId('checkbox-newname.go', { timeout: 10000 }).click({
      force: true
    })
    cy.get('[data-testid="checkbox-newname.go"] div').should(
      'have.css',
      'border-top-color',
      ENABLED_COLOUR
    )
