...
describe('Name detail view', () => {
  it('should redirect if searchTerm and normalized name does not match', () => {
    cy.visit(`${NAME_ROOT}/%E2%80%8Btest.go`)
    cy.wait(10000)
    cy.url().should('match', /\/test\.go\/?/)
  })
  it('can see list of top level domains from [root]', () => {
    cy.visit(`${NAME_ROOT}/[root]/subdomains`)
    cy.queryByTestId('go', { timeout: 30000 }).should('exist')
  ...
  it('can change the resolver to the public resolver', () => {
    cy.visit(`${NAME_ROOT}/superawesome.go`)
  ...
  it(`prevents user from adding a record that isn't an address`, () => {
    cy.visit(`${NAME_ROOT}/notsoawesome.go`)
  ...
  it('can add record', () => {
    ...
    const text = 'Hello'
    const url = `${NAME_ROOT}/notsoawesome.go`
    cy.visit(url)
  ...
  it('can change the record', () => {
    const url = `${NAME_ROOT}/abittooawesome.go`
    cy.visit(url)
  ...
  it('cannot change deprecated ipns contenthash', () => {
    const url = `${NAME_ROOT}/abittooawesome2.go`
    cy.visit(url)
  ...
  it('can navigate to a subdomain', () => {
    cy.visit(`${NAME_ROOT}/subdomaindummy.go/subdomains`, { timeout: 10000 })
    cy.getByText('original.subdomaindummy.go', { timeout: 15000 }).click({
      force: true
    })
  })

  it('can add a subdomain', () => {
    const LABEL = 'sub1' // using the same subdomain label which is used at sub1.testing.go
    cy.visit(`${NAME_ROOT}/subdomaindummy.go/subdomains`, { timeout: 10000 })
    cy.wait(5000)
    ...
    cy.wait(1000)
    cy.visit(`${NAME_ROOT}/subdomaindummy.go/subdomains`)
      .getByText('subdomains', { exact: false, timeout: 10000 })
      .click({ force: true })

    cy.queryByText(`${LABEL}.subdomaindummy.go`, { timeout: 10000 }).should(
      'exist'
    )
  })

  it('can view records that the user does now own', () => {
    cy.visit(`${NAME_ROOT}/otherowner.go`, { timeout: 10000 })
